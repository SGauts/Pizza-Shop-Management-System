...
def validPassword(password):
    ...
    if letter == None:
        print("Password should contain at least 1 alphabet")
        return False
...
def validDate(dat):
    dat = re.split("[./-]",dat)
    if len(dat) != 3:
        print("Date should take exactly 3 parameters")
        return False
    try:
        day,month,year = list(map(int,dat))
    except Exception as e:
        print("Invalid entry should be only Integer")
        return False
    if not(year>=1000 and year<=9999):
        print("Correct year format is 'yyyy' and between 1950 - 2018")
        return False
    if not(month>=1 and month<=12):
        print("Invalid month , should be between 1 - 12")
        return False
    if day<=0:
    ...
    if day>mon[month]:
        print(month," month cannot have more than ",mon[month]," days")
        return False
    return True
